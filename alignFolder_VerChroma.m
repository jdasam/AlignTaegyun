function [] = alignFolder_VerChroma( dirFolder )
...
if exist(dirScoreAudio,'file');
    ...
    if exist(dirScoreFBR,'file');
        ...
        scoreFBR=prepareAudio(dirScoreAudio,sampleRate, useChroma);
...
for nTarget=1:numel(fileNameTargets);
    ...
    if exist(dirTargetFBR,'file');
        ...
        targetFBR=prepareAudio(dirTargetAudio,sampleRate, useChroma);
...
%         save(dirDTWPath,'p','q','cost');
    ...
    dirGTmidi = fullfile(dirFolder, strcat(tempName,'.mid'));
    if exist(dirGTmidi, 'file');
        midiMat = readmidi_java(dirGTmidi);
    end
    
    
    alignOnset=alignOnset/sampleRate;
    errorVector = abs(midiMat(:,6)' - alignOnset);
    fileNameCSV=strcat(tempName,'.csv');
    dirCSV=fullfile(dirFolder,fileNameCSV);
    csvwrite(dirCSV,errorVector);
